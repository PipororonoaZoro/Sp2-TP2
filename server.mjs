    ...
    try
    {
        ...
        (
            // La cadena de conexión a MongoDB Atlas, que incluye el nombre de usuario, la contraseña, el clúster(o servidor) y el nombre de la base de datos.
             "mongodb+srv://grupo-14:grupo-14@cluster0.blryo.mongodb.net/NodeMod3Cohorte5",
        );
...
const superheroSchema = new mongoose.Schema(
    ...
    // La aplicación { collection: "dbsuperheroes" } se utiliza para espicificar el nombre de la colección en MongoDB
    // donde se almacenarán los documentos de este modelo.
    { collection: "Grupo-14" }
    // { collection: "nuevacollecion" },
);
// Se crea un modelo de Mongoose llamado "SuperHero" basado en el esquema difinifo anteriormente.
// para poder interactuar con la colleción "dbsuperheroes" en MongoDB utilizando este modelo.
...
async function obtenerTodos()
{
    ...
    // El resultado será un array de ibjetos que representan a cada superhéroe almacenado en la base de datos.
    return await SuperHero.find();
}

async function buscarPorPlaneta(planetaOrigen)
{
    // SuperHero.find({ planetaOrigen }) es una consulta que busca todos los documentos en la colección "dbsuperheroer" donde el campo "planetaOrigen"
    // coincideccon el valor proporcionado
...
{
    // Se crea una nueva instancia del modelo SuoerHero con los datos proporcionados.
...
{
    // SuperHero.update.One() es un método de Mongoose que se utiliza para actualizar un documento en la colección "dbsuperheroes"
    return await SuperHero.updateOne({ nombreSuperHeroe }, { $set: datosActualizados },);
}
...
app.get("/api/heroes", async (request, response) =>
{
    ...
    // incluyendo detalles como los encabezados, el cuerpo de la solicitud, los parámetros, etc.
